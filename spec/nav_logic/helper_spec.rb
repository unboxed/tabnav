...
describe NavLogic::Helper, :type => :helper do
  ...
  describe "render_tabnav" do
    before :each do
      ...
      @template = helper
      @navbar = NavLogic::Navbar.new(@template, {})
      NavLogic::Navbar.stub!(:new).and_return(@navbar)
      helper.output_buffer = ''
    end
    it "should create a new navbar with the template and params and yield it to the block" do
      params[:foo] = "bar"
      NavLogic::Navbar.should_receive(:new).with(@template, {"foo" => "bar"}, {}).and_return(@navbar)
    ...
    it "should create a new navbar with any passed options" do
      NavLogic::Navbar.should_receive(:new).with(@template, {}, {:class => "my_class", :id => "my_id"}).and_return(@navbar)
  ...
  it "should be included in ActionController::Base's helpers" do
    ActionController::Base.helpers.should be_a(NavLogic::Helper)
  end
end
